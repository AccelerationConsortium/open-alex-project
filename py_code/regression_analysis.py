##THIS IS THE old CODE. CAN REMOVE THIS ONCE I KNOW THAT DATASET REDO IS DONE
...
"""
Baseline Regression Analysis - Single Script
Uses pre-filtered 490K dataset (filtering already applied during dataset creation)
Runs Models 1-6, 8-9, and 21-26
"""
...
# Input file - USE FILTERED DATASET (venue matching already applied)
FILTERED_DATA = PROJECT_DIR / "data" / "regression/test" / "regression_dataset_filtered.csv"

# Output directory
OUTPUT_DIR = PROJECT_DIR / "data_regression_results" / "test" 
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
...
def load_data():
    """Load pre-filtered 490K dataset"""
    
    print("="*70)
    print("LOADING DATA")
    print("="*70)
    
    print(f"\n📂 Loading: {FILTERED_DATA}")
    print("   (This dataset is already filtered to SDL venues)")
    
    df = pd.read_csv(FILTERED_DATA, low_memory=False)
...
def main():
    ...
    print(f"\n⚙️  Using pre-filtered 490K dataset")
    print(f"   (SDL venue matching already applied)")
    ...
    print(f"  • model8.txt, model9.txt (Corresponding author)")
    print(f"  • model21.txt through model26.txt (AI/Robotics)")
    print(f"  • model_comparison_all.csv")
    print(f"  • coefficients_baseline.csv")
    print(f"  • fixed_effects.csv")
    print(f"  • subsample_summary.csv")
